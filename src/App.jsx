...
export default function App() {
  ...
  const fetchBookings = useCallback(async () => {
    const { data } = await supabase.from('bookings').select('slot_id, booked_by')
    if (data) {
      const map = {}
  ...
  const toggleSlot = async (slotId) => {
    ...
    if (owner === myName) {
      ...
      await supabase.from('bookings').upsert({ slot_id: slotId, booked_by: myName })
    }
    await fetchBookings()
    setSaving(null)
  }
    ...
    return (
      <div className="screen name-screen">
        <div className="name-card">
          <p className="name-subtitle">שבוע 7–12 יוני 2026</p>
      ...
      <header className="app-header">
        <div className="header-top">
          ...
          <div className="header-meta">
            <span className="week-label">7–12 יוני 2026</span>
          </div>
        </div>
